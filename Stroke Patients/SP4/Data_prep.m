...
marker_indices = [  3100, 3000, 2500, ...     GM:   0 10 20 
                    2450, 2400, 2580, ...     SOL:  0 10 20 
                    2100, ...                 SOL2: 20
                    2100, ...                 SOL3: 20
                    2400, 2300, 2250, ...     TA1:  0 20  
                    2250, 1900, 2000 ...      TA2:  0 10 20 
                    ];
% Same order as muscle_names
frequencies = [     16, 16, 16, ...     GM:   0 10 20 
                    16, 16, 16, ...     SOL:  0 10 20 
                    16, ...                 SOL2: 20
                    16, ...                 SOL3: 20
                    33, 33, 33, ...     TA1:  0 20  
                    33, 33, 33 ...      TA2:  0 10 20 
                    ];
US_muscle_labels = {'med_gas_r', 'med_gas_r', 'med_gas_r',  ...
                    'soleus_r', 'soleus_r', 'soleus_r', ...
                    'soleus_r', ...
                    'soleus_r',  ...
                    'tib_ant_r', 'tib_ant_r', 'tib_ant_r', ...
                    'tib_ant_r', 'tib_ant_r', 'tib_ant_r'};
...
for i=1:length(muscle_names)
    ...
    if length(GM.values) == length(SOL.values)
        EMG_raw = [GM.values, SOL.values, TA.values];
    else
        EMG_raw = [GM.values(1:end-1, :), SOL.values, TA.values(1:end-1, :)];
    end
...
for i=1:length(muscle_names)
    ...
    %EMG_t_full = GM.times;
    %---Delay and cut data----
    EMG_N_freq = 1/GM.interval;
    C_downsampling = round(EMG_N_freq/US_rate);
    ...
    R_time = downsample(EMG_t_full, C_downsampling);
    %---Delay and cut data----
